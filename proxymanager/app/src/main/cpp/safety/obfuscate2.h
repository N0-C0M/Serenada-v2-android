...
#ifndef REE_V3_OBFUSCATE2_H
#define REE_V3_OBFUSCATE2_H

#endif //REE_V3_OBFUSCATE2_H
/* --------------------------------- ABOUT -------------------------------------
...
namespace ay
{
    ...
    constexpr key_type generate2_key(key_type seed)
    {
        // Use the MurmurHash3 64-bit finalizer to hash our seed
        key_type key = seed;
        key ^= (key >> 33);
        key *= 0xffff3fd7ed558ccd;
        key ^= (key >> 33);
        key *= 0xc4ceb9f81a85e253;
        key ^= (key >> 33);
    ...
    constexpr void cipher2(char* data, size_type size, key_type key)
    {
    ...
    template <size_type N, key_type KEY>
    class obfuscated2_data
    {
    public:
        obfuscated2_data(const obfuscator2<N, KEY>& obfuscator2)
        {
    ...
    // Handles immediate decryption of an encrypted string at runtime
    template <size_type N, key_type KEY>
    class obfuscated_data_immediate
    {
    public:
        obfuscated_data_immediate(const obfuscator<N, KEY>& obfuscator)
        {
            // Copy obfuscated data
            for (size_type i = 0; i < N; i++)
            {
            ...
            // Immediately decrypt the data
            cipher(m_data, N, KEY);
        }

        ~obfuscated_data_immediate()
        {
            // Zero m_data to remove it from memory
            for (size_type i = 0; i < N; i++)
            {
        ...
        // Returns a pointer to the plain text string
        operator char*()
        {
            return m_data;
        }
    ...
    private:
        // Local storage for the decrypted string
        char m_data[N];
    };

    // This function exists purely to extract the number of elements 'N' in the
    // array 'data'
    template <size_type N, key_type KEY = AY_OBFUSCATE2_DEFAULT_KEY>
...
// char*
#define OBFUSCATE2_KEY(data, key) \
...
// Obfuscates the string 'data' at compile-time and returns a reference to a
// ay::obfuscated_data_immediate object with global lifetime that is immediately
// decrypted and implicitly convertable to a char*
#define OBFUSCATE2V(data) OBFUSCATE2V_KEY(data, AY_OBFUSCATE_DEFAULT_KEY)

// Obfuscates the string 'data' with 'key' at compile-time and returns a
// reference to a ay::obfuscated_data_immediate object with global lifetime that
// is immediately decrypted and implicitly convertable to a char*
#define OBFUSCATE2V_KEY(data, key) \
	[]() -> ay::obfuscated_data_immediate<sizeof(data)/sizeof(data[0]), key>& { \
		static_assert(sizeof(decltype(key)) == sizeof(ay::key_type), "key must be a 64 bit unsigned integer"); \
		static_assert((key) >= (1ull << 56), "key must span all 8 bytes"); \
		constexpr auto n = sizeof(data)/sizeof(data[0]); \
		constexpr auto obfuscator = ay::make_obfuscator<n, key>(data); \
		static auto obfuscated_data = ay::obfuscated_data_immediate<n, key>(obfuscator); \
		return obfuscated_data; \
	}()
